...
function statusChip(row: RunRow) {
  if (row.failed > 0) {
    return <span className="tz-chip" style={{ background: "rgba(217,59,59,0.10)", color: "var(--tz-loss)", borderColor: "rgba(217,59,59,0.35)" }}>✗ {row.failed} fail</span>;
  }
  if (row.warned > 0) {
    return <span className="tz-chip tz-chip-amber">⚠ {row.warned} warn</span>;
  }
  return <span className="tz-chip tz-chip-acid"><span className="tz-chip-dot" />All green</span>;
}
...
export default async function QaPage() {
  ...
  return (
    <>
      <div className="tz-topbar">
        <div>
          ...
          <div className="tz-topbar-sub">
            Pre-deploy go-live readiness gate · 50+ checks across Build, Env, Security, Functional, SEO, Database, Pricing, UX
          </div>
        </div>
        ...
        <div className="tz-card-head">
          <div>
            ...
            <div className="tz-card-sub">
              Probes this deploy ({process.env.VERCEL_GIT_COMMIT_SHA?.slice(0, 7) || "local"}) · {process.env.VERCEL_ENV || "local"} env. Run before every push.
            </div>
          </div>
        </div>
        <QaRunButton />
      ...
      {/* KPI strip — always visible from latest run */}
      {latest && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
          <div className="tz-kpi">
            <div className="tz-kpi-label">Total checks</div>
          ...
          <div className="tz-kpi">
            ...
            <div className="tz-kpi-value" style={{ color: "var(--tz-amber)" }}>{latest.warned}</div>
          </div>
          <div className="tz-kpi">
      ...
      {tableMissing && (
        <div className="tz-card mb-6" style={{
          ...
          <p className="text-[13.5px]" style={{ color: "var(--tz-ink-dim)" }}>
            Apply <code>supabase/migrations/020_qa_runs.sql</code> in the Supabase SQL editor, then refresh.
            You can still run the suite below — results will be in the response payload but won&apos;t persist until the table exists.
          </p>
        </div>
      )}
      ...
      <div className="tz-card" style={{ padding: 0, overflow: "hidden" }}>
        <div className="tz-card-head" style={{
          ...
          <div>
            ...
            <div className="tz-card-sub">
              Each row is a versioned snapshot. Click any run to drill into individual check results.
            </div>
          </div>
        </div>
        {rows.length === 0 ? (
          <p className="text-[13.5px] p-6" style={{ color: "var(--tz-ink-mute)" }}>
            No runs yet. Click <strong>Run QA suite</strong> above to generate the first one.
          </p>
        ) : (
            ...
            <table className="tz-table" style={{ minWidth: 820 }}>
              ...
              <tbody>
                {rows.map((r) => (
                  <tr key={r.id}>
                    <td>{statusChip(r)}</td>
                    <td className="font-mono text-[12px]" style={{ color: "var(--tz-ink-dim)" }}>{r.version}</td>
      ...
      <p className="mt-6 text-[10.5px] font-mono uppercase tracking-widest"
        ...
        Suite is read-only · zero side effects · safe to run in production
      </p>
    </>
  );
}
