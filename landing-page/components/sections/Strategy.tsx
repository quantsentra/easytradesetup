...
function ChartCard({ label, outcome, tag, tagColor, note }: typeof results[0]) {
  return (
    <div className="rounded-card border border-border bg-bg-surface overflow-hidden group
      hover:border-accent-blue/30 hover:-translate-y-2 hover:shadow-2xl hover:shadow-accent-blue/5
      ...
      <div className="relative aspect-video bg-bg-raised flex items-center justify-center overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-br from-accent-blue/5 to-accent-green/5
          group-hover:from-accent-blue/10 group-hover:to-accent-green/10 transition-all duration-500" />
        {/* Fake candle chart lines */}
        <svg className="absolute inset-0 w-full h-full opacity-15 group-hover:opacity-25 transition-opacity duration-300"
        ...
        <div className="relative z-10 text-center">
          <div className="text-2xl font-black text-accent-green group-hover:scale-110 transition-transform duration-300">{outcome}</div>
    ...
    <section id="strategy" className="py-14 sm:py-24 bg-bg-surface/40 border-y border-border">
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {results.map((r, i) => (
            <FadeIn key={r.label + r.outcome} delay={0.1 + i * 0.09}>
              <ChartCard {...r} />
            </FadeIn>
          ))}
        </div>

        <FadeIn delay={0.45} className="mt-10 rounded-card border border-border bg-bg-surface p-6 text-center">
          ...
          <a
            href="#pricing"
            className="mt-4 inline-block px-5 py-2.5 rounded-xl bg-accent-blue/10 border border-accent-blue/20 text-accent-blue text-sm font-semibold hover:bg-accent-blue/20 hover:-translate-y-0.5 transition-all duration-200"
          >
