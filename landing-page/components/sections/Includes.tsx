        ...
        <div className="grid sm:grid-cols-2 gap-4 max-w-3xl mx-auto">
          {items.map(({ icon, title, desc }, i) => (
            <FadeIn key={title} delay={0.1 + i * 0.09}>
              <div className="flex gap-4 bg-bg-surface border border-border rounded-card p-5 group
                hover:border-accent-blue/40 hover:-translate-y-1.5 hover:shadow-xl hover:shadow-accent-blue/5
                transition-all duration-300 h-full cursor-default">
                <span className="text-2xl flex-shrink-0 mt-0.5
                  group-hover:scale-125 group-hover:-rotate-6 transition-transform duration-300">{icon}</span>
                <div>
                  <div className="font-semibold text-ink group-hover:text-accent-blue transition-colors duration-200">{title}</div>
