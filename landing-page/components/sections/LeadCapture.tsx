  ...
  return (
    <section className="py-20 border-b border-border">
      <div className="max-w-xl mx-auto px-4 sm:px-6 text-center">
        {/* Icon */}
        <div className="w-12 h-12 rounded-2xl bg-accent-green/10 border border-accent-green/20 flex items-center justify-center mx-auto mb-6">
        ...
        {status === 'done' ? (
          ...
          <form onSubmit={handleSubmit} className="mt-8 flex flex-col sm:flex-row gap-3">
            <input
              ...
              required
              className="flex-1 bg-bg-surface border border-border rounded-xl px-4 py-3 text-sm text-ink placeholder:text-ink-faint focus:outline-none focus:border-accent-green/50 focus:ring-1 focus:ring-accent-green/20 transition-colors"
            />
            <button
              type="submit"
              disabled={status === 'loading'}
              className="flex-shrink-0 bg-accent-green text-black font-bold text-sm px-6 py-3 rounded-xl hover:bg-accent-green/90 transition-colors disabled:opacity-60 disabled:cursor-not-allowed shadow-glow-green"
            >
